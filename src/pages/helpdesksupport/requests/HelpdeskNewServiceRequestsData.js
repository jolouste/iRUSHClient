...
const HelpdeskNewServiceRequestsData = () => {
			...
			await instance
				...
				.catch(error => {
					if (error.response.status === 404) {
						navigate("/helpdesk/servicerequests");
					}
		...
		const getRequesterServiceRequest = async () => {
			...
			try {
				await instance
					...
					.then(response => {
						let serviceRequest = response.data.service;
						setServiceRequestData(serviceRequest);
					});
			} catch (error) {
				// if error occured while fetching data from the server then redirect to the 404 page
				if (error.response.status === 404) {
					toast.error(error.response.data.message);
					navigate("/helpdesksupport/servicerequests");
				}
				if (error.response.status === 401) {
					window.location.href = "/login";
				}
...
export default HelpdeskNewServiceRequestsData;
