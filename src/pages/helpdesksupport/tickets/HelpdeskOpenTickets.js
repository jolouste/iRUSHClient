...
import HelpdeskOpenTicketLists from "../../../components/ticketstats-helpdesk/HelpdeskOpenTicketsLists";
import instance from "../../../axios/axios";
import pdficon from "../../../images/img/pdficon.png";

const HelpdeskOpenTickets = ({ ticketNavId }) => {
	...
	useEffect(() => {
		const fetchOpenTickets = async () => {
			...
			await instance
				.get(
					`/tickets/helpdesk/assignedtickets/opentickets?priority=${priority.priority}&page=${page}&limit=${limit}&search=${search}&ticketCategory=${category.category}&dateFrom=${dateFrom.dateFrom}&dateTo=${dateTo.dateTo}`
				)
				.then(response => {
	...
	useEffect(() => {
		instance
			.get("/tickets/helpdesksupport/report/opentickets", {
				responseType: "blob",
			})
			.then(response => {
