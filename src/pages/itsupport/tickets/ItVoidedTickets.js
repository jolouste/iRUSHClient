...
import ItVoidedTicketsLists from "../../../components/ticketstats-it/ItVoidedTicketsLists";
import instance from "../../../axios/axios";
import pdficon from "../../../images/img/pdficon.png";

const ItVoidedTickets = ({ ticketNavId }) => {
	...
	useEffect(() => {
		const fetchRejectedTickets = async () => {
			...
			await instance
				.get(
					`/tickets/itsupport/assignedtickets/voidedtickets?priority=${priority.priority}&page=${page}&limit=${limit}&search=${search}&ticketCategory=${category.category}&dateFrom=${dateFrom.dateFrom}&dateTo=${dateTo.dateTo}`
				)
				.then(response => {
	...
	useEffect(() => {
		instance
			.get("/tickets/itsupport/report/voidedtickets", {
				responseType: "blob",
			})
			.then(response => {
...
export default ItVoidedTickets;
