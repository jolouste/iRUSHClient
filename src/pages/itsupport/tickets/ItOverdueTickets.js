...
import ItOverdueTicketsLists from "../../../components/ticketstats-it/ItOverdueTicketsLists";
import instance from "../../../axios/axios";
import pdficon from "../../../images/img/pdficon.png";

const ItOverdueTickets = ({ ticketNavId }) => {
	...
	useEffect(() => {
		const fetchOverdueTickets = async () => {
			...
			await instance
				.get(
					`/tickets/itsupport/assignedtickets/overduetickets?priority=${priority.priority}&page=${page}&limit=${limit}&search=${search}&ticketCategory=${category.category}&dateFrom=${dateFrom.dateFrom}&dateTo=${dateTo.dateTo}`
				)
				.then(response => {
	...
	useEffect(() => {
		instance
			.get("/tickets/itsupport/report/overduetickets", {
				responseType: "blob",
			})
			.then(response => {
...
export default ItOverdueTickets;
