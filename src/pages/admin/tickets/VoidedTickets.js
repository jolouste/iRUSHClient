...
import Voidedticketslist from "../../../components/tickestats/voidedticket/Voidedticketslist";
import PieChart from "../../../components/tickestats/voidedticket/charts/PieChart";
import DoughnutChart from "../../../components/tickestats/voidedticket/charts/DoughnutChart";
import BarChart from "../../../components/tickestats/voidedticket/charts/BarChart";
import instance from "../../../axios/axios";
import pdficon from "../../../images/img/pdficon.png";

const VoidedTickets = ({ ticketNavId }) => {
	...
	useEffect(() => {
		const fetchVoidedTickets = async () => {
			...
			await instance
				.get(
					`/tickets/voidedtickets?priority=${priority.priority}&page=${page}&limit=${limit}&search=${search}&ticketCategory=${category.category}&dateFrom=${dateFrom.dateFrom}&dateTo=${dateTo.dateTo}`
				)
				.then(response => {
					const filteredTickets = response.data.filteredTickets;
					const limit = response.data.limit;
					const total = response.data.total;
					setVoidedTickets(filteredTickets);
					//for pagination
					setLimit(limit);
					setTotal(total);
				})
	...
	useEffect(() => {
		instance
			.get("/tickets/report/voidedtickets", {
				responseType: "blob",
			})
			.then(response => {
			...
			<>
				<div className="ticketstats-content__container">
					...
					<Voidedticketslist
						...
						loading={loading}
						setVoidedTickets={setVoidedTickets}
					/>
