...
import Openticketslist from "../../../components/tickestats/openticket/Openticketslist";
import PieChart from "../../../components/tickestats/openticket/charts/PieChart";
...
const OpenTickets = ({ ticketNavId }) => {
	...
	useEffect(() => {
		const fetchOpenTickets = async () => {
			...
			await instance
				.get(
					`/tickets/opentickets?priority=${priority.priority}&page=${page}&limit=${limit}&search=${search}&ticketCategory=${category.category}&dateFrom=${dateFrom.dateFrom}&dateTo=${dateTo.dateTo}`
				)
				.then(response => {
