...
const NewServiceRequestslist = ({
	...
	console.log(rejectedServiceRequestsData);
	const [order, setOrder] = useState("ASC");

	const sorting = col => {
		if (order === "ASC") {
			const sorted = [...rejectedServiceRequestsData].sort((a, b) =>
				a[col].toLowerCase() > b[col].toLowerCase() ? 1 : -1
			);
			setRejectedServiceRequestsData(sorted);
			setOrder("DSC");
		}
		if (order === "DSC") {
			const sorted = [...rejectedServiceRequestsData].sort((a, b) =>
		...
		<>
			<div className="servicerequestscontainer-service__servicelists">
				{loading ? (
					...
					<>
						{rejectedServiceRequestsData.length === 0 ? (
							<div className="noavailability__container">
								<h3> No Rejected Service Requests available </h3>
							</div>
						) : (
							<>
								<table className="table-container__tickets">
									...
									<tbody>
										{rejectedServiceRequestsData.map((service, index) => {
