...
const PieChart = () => {
	...
	useEffect(() => {
		const fetchChartData = async () => {
			...
			const clientunits = [
				...new Set(data.service.map(service => service.clientUnit)),
			];
			...
			const serviceClientUnitData = [
				...
				return data.service.filter(service => service.clientUnit === clientUnit)
					.length;
			});
					...
					<Pie
						...
						options={{
							...
							plugins: {
								...
								title: {
									display: true,
									text: "CURRENT NEW SERVICE REQUESTS PER TO UNIT",
								},
							},
						}}
					/>
