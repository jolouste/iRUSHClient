...
const DoughnutChart = () => {
	...
	useEffect(() => {
		const fetchChartData = async () => {
			...
			const priorityData = [
				...new Set(data.openTickets.map(ticket => ticket.priority)),
			];
			...
			const ticketPriorityData = [
				...
				return data.openTickets.filter(ticket => ticket.priority === priority)
					.length;
			});
					...
					<Doughnut
						...
						options={{
							...
							plugins: {
								...
								title: {
									display: true,
									text: "CURRENT OPEN TICKETS ACCORDING TO PRIORITY",
								},
							},
						}}
					/>
