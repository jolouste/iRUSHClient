...
const Voidedticketslist = ({ voidedTickets, loading, setVoidedTickets }) => {
	const [order, setOrder] = useState("ASC");

	const sorting = col => {
		if (order === "ASC") {
			const sorted = [...voidedTickets].sort((a, b) =>
				a[col].toLowerCase() > b[col].toLowerCase() ? 1 : -1
			);
			setVoidedTickets(sorted);
			setOrder("DSC");
		}
		if (order === "DSC") {
			const sorted = [...voidedTickets].sort((a, b) =>
						...
						{voidedTickets.length === 0 ? (
							...
							<>
								<table className="table-container__tickets">
									...
									<tbody>
										{voidedTickets.map((ticket, index) => {
											const { _id: id } = ticket;
											return (
...
export default Voidedticketslist;
