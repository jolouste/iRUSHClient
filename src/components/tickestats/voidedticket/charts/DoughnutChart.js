...
const DoughnutChart = () => {
	...
	useEffect(() => {
		const fetchChartData = async () => {
			...
			const priorityData = [
				...new Set(data.voidedTickets.map(ticket => ticket.priority)),
			];
			...
			const ticketPriorityData = [
				...
				return data.voidedTickets.filter(ticket => ticket.priority === priority)
					.length;
			});
					...
					<Doughnut
						...
						options={{
							...
							plugins: {
								...
								title: {
									display: true,
									text: "CURRENT RESOLVED TICKETS ACCORDING TO PRIORITY",
								},
							},
						}}
					/>
