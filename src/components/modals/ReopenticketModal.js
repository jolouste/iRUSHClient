...
import { Requestservicesuccess } from "./Requestservicesuccess";
import Circularspinner from "../spinner/Circularspinner";
import instanceNoAuth from "../../axios/instanceNoAuth";

function ReopenticketModal({ setReopenticketOpenModal }) {
	...
	const closeModal = e => {
		if (modalRef.current === e.target) {
			setReopenticketOpenModal(false);
		}
	};
	return (
		<>
											...
											<div className="modal-btns__wrapper">
												<div className="modal-btn__close">
													<Buttons
														...
														onClick={() => setReopenticketOpenModal(false)}
													>
														CLOSE
													</Buttons>
												</div>
