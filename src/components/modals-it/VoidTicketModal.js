...
const VoidTicketModal = ({ modalOpen }) => {
	...
	const [voidTicket, setVoidTicket] = useState({
		voidReason: "",
		remarks: "",
	});
	...
	const handleChange = ({ currentTarget: input }) => {
		setVoidTicket({ ...voidTicket, [input.name]: input.value });
	};
	...
	const handleSubmit = async e => {
		...
		try {
			await instance
				.post(
					`/tickets/itsupport/assignedtickets/voidticket/${param.id}`,
					voidTicket
				)
				.then(response => {
							...
							<div className="rejectrequestcontainer-modal__wrapper">
								<div className="rejectrequestcontainer-modal__paragraph">
									<p className="rejectrequestmodal-text">
										Before submitting as voided, please state the reason why
									</p>
								</div>
										...
										<div className="rejectreason__requestservice">
											...
											<div className="rejectreason-container__dropdown">
												<select
													...
													name="voidReason"
													value={voidTicket.voidReason}
												>
													<option disabled label="Choose one"></option>
													{voidData.map((voidreason, key) => (
														<option key={key} value={voidreason.voidReasonName}>
															{voidreason.voidReasonName}
														</option>
													))}
												</select>
											</div>
