...
const PieChart = () => {
	...
	useEffect(() => {
		const fetchChartData = async () => {
			...
			const categories = [
				...new Set(data.ticket.map(ticket => ticket.ticketCategory)),
			];
			...
			const ticketCategoryData = [
				...
				return data.ticket.filter(ticket => ticket.ticketCategory === category)
					.length;
			});
					...
					<Pie
						...
						options={{
							...
							plugins: {
								...
								title: {
									display: true,
									text: "TICKETS CREATED ACCORDING TO CATEGORY",
								},
							},
						}}
					/>
